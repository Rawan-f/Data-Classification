...
def load_data(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            if values[-1] in ['class-1', 'class-2']:
...
def train_and_evaluate_pair(classifier_name, train_data, train_labels, test_data, test_labels):
    ...
    most_discriminative_feature = np.argmax(np.abs(perceptron.weights))
    print(f"The most discriminative feature is Feature {most_discriminative_feature + 1} with weight: {perceptron.weights[most_discriminative_feature]:.2f}")

# Load train and test data for class 1 and class 2
train_data, train_labels = load_data("train.data")
test_data, test_labels = load_data("test.data")

# Train and evaluate binary Perceptron for class 1 and class 2
train_and_evaluate_pair("Class 1 vs. Class 2", train_data, train_labels, test_data, test_labels)